...
public class SearchService : ISearchService
{
    ...
    private bool _enableAI;  // TODO
    private string AI_API_URL; // 大先生 "https://chat.zju.edu.cn/api/ai/v1/chat/completions"
    ...
    public async Task<List<(string title, string url, string date, int number)>> getSearchResult(string searchText)
    {
        if (_enableAI)
        {
            try
            {
                ...
                var allResults = new List<(string title, string url, string date, int number)>();
    ...
    private async Task<List<(string title, string url, string date, int number)>> SearchWithSourcesAsync(string searchText)
    {
    ...
    // TODO
    public void setEnableAI(bool enableAI) { _enableAI = enableAI; }
