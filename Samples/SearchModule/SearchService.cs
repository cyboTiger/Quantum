...
public class SearchService : ISearchService
{
    ...
    public int pingSource(int source)
    {
        ...
        var rootUrl = new Uri(sourceRetriever.SourceUrl).Host;
        ...
        try
        {
            ...
            {
                PingReply reply = pingSender.Send(rootUrl);
                if (reply.Status == IPStatus.Success)
                {
                    _logger.LogInformation($"Ping to {rootUrl} succeeded. Roundtrip time: {reply.RoundtripTime} ms");
                    return (int)reply.RoundtripTime;
                }
                else
                {
                    _logger.LogWarning($"Ping to {rootUrl} failed. Status: {reply.Status}");
                    return -1;
                }
            }
        }
        ...
        {
            _logger.LogError($"Ping exception: {e.Message}, url={rootUrl}");
            return -1;
        }
    }
    ...
    public string sourceName(int source)
    {
        ...
        var sourceKey = sourceMap.Keys.ElementAt(source);
        return sourceMap[sourceKey].SourceName;
    }
    ...
    public bool getEnableSource(int source)
    {
        ...
        var sourceKey = sourceMap.Keys.ElementAt(source);
        _logger.LogInformation($"Source {sourceKey} is {(_sourceRetrieve[sourceKey] ? "enabled" : "disabled")}.");
        return _sourceRetrieve[sourceKey];
    }
